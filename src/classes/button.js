export const BUTTON_STYLES = [
  'center-nav-link p-2 cursor-pointer hover:opacity-75 relative z-10',
  'opacity-0 animate-fade-in translate-y-4 transition-all duration-500',
  'font-kode text-sm text-black-dull dark:text-white-dull text-center ',
  'border border-transparent hover:border-black dark:hover:border-white rounded-xs',
  'px-8 py-1 m-2 my-3 bg nav-borders'
];

export const ICON_BUTTON_STYLES = [
  'center-nav-link p-2 cursor-pointer hover:opacity-75 relative z-10',
  'opacity-0 animate-fade-in translate-y-4 transition-all duration-500',
  'font-kode text-sm text-white-dull text-center ',
  'border border-transparent hover:border-white rounded-xs',
  'px-8 py bg'
];